//--------------------------------------------------------
//   EtherShield example: simpleClient Pachube with DHCP + DNS + RF12
//
...
#define PACHUBEAPIKEY "www.pachube.com\r\nX-PachubeApiKey: xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" 
...
{
  Serial.begin(9600);
  Serial.println("EtherShield_simpleClient_Pachube_DHCP_DNS_RF12");
...
void loop()
{
  ...
  if (rf12_recvDone() && rf12_crc == 0 && (rf12_hdr & RF12_HDR_CTL) == 0 && rf12_len==sizeof(Payload) ) 
  {
    ...
    // Construct CSV
    Serial.println("CSV:");
    sprintf(str,"%d,%d,%d,%d,%d,%d,%d",emontx.ct1, emontx.ct2, emontx.nPulse, emontx.temp1, emontx.temp2, emontx.temp3, emontx.supplyV);
    Serial.println(str);
